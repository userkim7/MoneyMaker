...
class game_set:
    def __init__(self):
        ...
        self.level_list=['Easy','Normal','Hard','Insane']
        self.character_list=[{'name':'Alex','stats':{'energy':5,'money':500,'health':50,'strength':3,'agility':3,'wisdom':3,'talent':3,'language':3,'luck':3,'badluck':3,'fame':0,'item':[]}},
                             ...
                             {'name':'Winne','stats':{'energy':20,'money':5000,'health':10,'strength':10,'agility':10,'wisdom':10,'talent':10,'language':10,'luck':10,'badluck':0,'fame':0,'item':['Cheater`s Hat','Linger']}},
    ...
    def setting_game(self,setting):
        ...
        if self.background_num==0:
            if self.level=='Normal': pass
            elif self.level=='Easy': self.tax_mul=[2.8,14]
            elif self.level=='Hard': self.tax_mul=[20,60]
            elif self.level=='Insane': self.tax_mul,self.tax_add=[20,60],[10/100,15/100]
        elif self.background_num==1:
            if self.level=='Normal': self.tax_mul=[6,60]
            elif self.level=='Easy': self.tax_mul=[3,16]
            elif self.level=='Hard': self.tax_mul=[22,64]
            elif self.level=='Insane': self.tax_mul,self.tax_add=[22,64],[10/100,15/100]
...
class event:
    def __init__(self):
        ...
        self.good_event_list=[
            ['PickMoney','PickClover','Party','TakeMushroom','Rainbow'],
            ['PickMoney','PickClover','Party','TakeMushroom','Rainbow']
            ]
        self.bad_event_list=[
            ['DropMoney','Fire'],
        ...
        self.item_list=[{'name':'Cheater`s Hat','stats':{'durability':99,'health':-5},'price':None},
                        {'name':'MoonStone','stats':{'durability':10,'health':1},'price':200},
                        {'name':'Linger','stats':{'durability':3,'health':5},'price':320},
                        {'name':'NoteBook','stats':{'durability':7,'wisdom':1},'price':120},
                        {'name':'Dictionary','stats':{'durability':3,'wisdom':3},'price':200},
                        {'name':'Adronalin','stats':{'durability':3,'strength':3,'health':-2},'price':280},
                        {'name':'WierdMushroom','stats':{'durability':1,'random':3},'price':None},
                        {'name':'Sneakers','stats':{'durability':7,'agility':1},'price':120},
                        {'name':'Booster','stats':{'durability':1,'agility':5,'health':-2},'price':120},
                        {'name':'Translater','stats':{'durability':10,'language':1},'price':280},
                        {'name':'Hammer','stats':{'durability':7,'strength':1,'talent':1},'price':200},
                        {'name':'Glasses','stats':{'durability':10,'wisdom':1,'talent':1,'health':-1},'price':280},
                        {'name':'Scissor','stats':{'durability':3,'talent':2},'price':160},
                        {'name':'Mushroom','stats':{'durability':1,'health':10,'strength':5,'agility':-5},'price':400},
                        {'name':'Clover','stats':{'durability':7,'luck':7,'badluck':-7},'price':None},
                        {'name':'Basketball','stats':{'durability':5,'strength':2,'agility':2},'price':240},
                        {'name':'Rainbow','stats':{'durability':1,'strength':7,'agility':7,'wisdom':7,'talent':7,'language':7},'price':None}
                        ]
    ...
    def make_list(self):
        if gs.background_num==0:
            ...
            for _ in range(4): self.utility_list+=[{'name':'EnergyDrink_I','stats':{'energy':1,'money':-20}},
            ...
            for _ in range(3): self.utility_list+=[{'name':'Medicine_I','stats':{'money':-80,'health':10}}
                                                   ] 
            for _ in range(2): self.utility_list+=[{'name':'EnergyDrink_II','stats':{'energy':2,'money':-40}},
            ...
            self.utility_list+=[{'name':'Medicine_II','stats':{'money':-160,'health':25}}
                                ]
        elif gs.background_num==1:
            for _ in range(5): self.utility_list+=[{'name':'Brush','stats':{'money':50}}
                                                   ] 
            for _ in range(4): self.utility_list+=[{'name':'Run_I','stats':{'energy':-1,'agility':1}},
                                                   {'name':'Carpent_I','stats':{'energy':-1,'talent':1}},
                                                   {'name':'Talk_I','stats':{'energy':-1,'language':1}}
                                                   ] 
            for _ in range(3): self.utility_list+=[{'name':'EnergyDrink_I','stats':{'energy':1,'money':-120}},
                                                   {'name':'Delivery_I','stats':{'energy':-2,'strength':1,'agility':1}},
                                                   {'name':'Internet_I','stats':{'energy':-1,'health':-3,'wisdom':1,'language':1}},
                                                   {'name':'Medicine_I','stats':{'money':-275,'health':12}}
                                                   ] 
            for _ in range(2): self.utility_list+=[{'name':'EnergyDrink_II','stats':{'energy':2,'money':-100}},
                                                   {'name':'PartTimeJob','stats':{'energy':-2,'money':350}},
                                                   {'name':'Read_I','stats':{'energy':-1,'wisdom':1}},
                                                   {'name':'Talk_II','stats':{'energy':-2,'language':2}},
                                                   {'name':'GoodNight','stats':{'next_energy':1}},
                                                   {'name':'MakeBed','stats':{'energy':-1,'next_energy':2}},
                                                   {'name':'Volunteer','stats':{'energy':-1,'money':-50,'fame':1}},
                                                   {'name':'Steal','stats':{'energy':-1,'money':300,'fame':-1}}
                                                   ] 
            self.utility_list+=[{'name':'Medicine_II','stats':{'money':-200,'health':22}},
                                {'name':'Delivery_II','stats':{'energy':-3,'strength':1,'agility':2}},
                                {'name':'Internet_II','stats':{'energy':-2,'health':-7,'wisdom':2,'language':2}}
                                ]
    ...
    def utility_value(self,utility):
        ...
        if 'energy' in utility['stats'].keys() and ma.player_energy<-1*utility['stats']['energy']: return 1
        if 'money' in utility['stats'].keys() and gs.money<-1*utility['stats']['money']: return 1
        if 'energy' in utility['stats'].keys(): ma.player_energy+=utility['stats']['energy']
        if 'next_energy' in utility['stats'].keys(): ma.next_energy+=utility['stats']['next_energy']
        if 'money' in utility['stats'].keys(): gs.money+=utility['stats']['money']
        if 'health' in utility['stats'].keys(): gs.health+=utility['stats']['health']
        if 'strength' in utility['stats'].keys(): gs.strength+=utility['stats']['strength']
        if 'agility' in utility['stats'].keys(): gs.agility+=utility['stats']['agility']
        if 'wisdom' in utility['stats'].keys(): gs.wisdom+=utility['stats']['wisdom']
        if 'talent' in utility['stats'].keys(): gs.talent+=utility['stats']['talent']
        if 'language' in utility['stats'].keys(): gs.language+=utility['stats']['language']
        if 'luck' in utility['stats'].keys(): gs.luck+=utility['stats']['luck']
        if 'badluck' in utility['stats'].keys(): gs.badluck+=utility['stats']['badluck']
        if 'fame' in utility['stats'].keys(): gs.fame+=utility['stats']['fame']
        for stats in utility['stats'].items():
            if stats[1]>=0: print(f'{stats[0]}...+{stats[1]}')
            else: print(f'{stats[0]}...{stats[1]}')
            
    def work_value(self,work):
        if not work: return 1
        self.work_value_list=[
            {'name':'Load_I','stats':{'earn':round(gs.health*(gs.strength+ma.strength)/14,2)+self.tips(gs.strength+ma.strength),'energy':-1,'strength':3}},
            {'name':'Mail_I','stats':{'earn':round(gs.health*(gs.agility+ma.agility)/14,2)+self.tips(gs.agility+ma.agility),'energy':-1,'agility':3}},
            {'name':'Library_I','stats':{'earn':round(gs.health*(gs.wisdom+ma.wisdom)/14,2)+self.tips(gs.wisdom+ma.wisdom),'energy':-1,'wisdom':3}},
            {'name':'Curve_I','stats':{'earn':round(gs.health*(gs.talent+ma.talent)/14,2)+self.tips(gs.talent+ma.talent),'energy':-1,'talent':3}},
            {'name':'Speech_I','stats':{'earn':round(gs.health*(gs.language+ma.language)/14,2)+self.tips(gs.language+ma.language),'energy':-1,'language':3}},
            {'name':'Load_II','stats':{'earn':round(gs.health*(gs.strength+ma.strength)/6,2)+self.tips(gs.strength+ma.strength),'energy':-2,'strength':7}},
            {'name':'Mail_II','stats':{'earn':round(gs.health*(gs.agility+ma.agility)/6,2)+self.tips(gs.agility+ma.agility),'energy':-2,'agility':7}},
            {'name':'Library_II','stats':{'earn':round(gs.health*(gs.wisdom+ma.wisdom)/6,2)+self.tips(gs.wisdom+ma.wisdom),'energy':-2,'wisdom':7}},
            {'name':'Curve_II','stats':{'earn':round(gs.health*(gs.talent+ma.talent)/6,2)+self.tips(gs.talent+ma.talent),'energy':-2,'talent':7}},
            {'name':'Speech_II','stats':{'earn':round(gs.health*(gs.language+ma.language)/6,2)+self.tips(gs.language+ma.language),'energy':-2,'language':7}},
            {'name':'Load_III','stats':{'earn':round(gs.health*(gs.strength+ma.strength)/4,2)+self.tips(gs.strength+ma.strength),'energy':-3,'strength':13}},
            {'name':'Mail_III','stats':{'earn':round(gs.health*(gs.agility+ma.agility)/4,2)+self.tips(gs.agility+ma.agility),'energy':-3,'agility':13}},
            {'name':'Library_III','stats':{'earn':round(gs.health*(gs.wisdom+ma.wisdom)/4,2)+self.tips(gs.wisdom+ma.wisdom),'energy':-3,'wisdom':13}},
            {'name':'Curve_III','stats':{'earn':round(gs.health*(gs.talent+ma.talent)/4,2)+self.tips(gs.talent+ma.talent),'energy':-3,'talent':13}},
            {'name':'Speech_III','stats':{'earn':round(gs.health*(gs.language+ma.language)/4,2)+self.tips(gs.language+ma.language),'energy':-3,'language':13}}
            ]
        for Dict in self.work_value_list:
            if Dict['name']==work:
                work=Dict
                break
        if ma.player_energy<-1*work['stats']['energy']: return 1
        if 'health' in work['stats'].keys() and gs.health<work['stats']['health']: return 1
        if 'strength' in work['stats'].keys() and gs.strength<work['stats']['strength']: return 1
        if 'agility' in work['stats'].keys() and gs.agility<work['stats']['agility']: return 1
        if 'wisdom' in work['stats'].keys() and gs.wisdom<work['stats']['wisdom']: return 1
        if 'talent' in work['stats'].keys() and gs.talent<work['stats']['talent']: return 1
        if 'language' in work['stats'].keys() and gs.language<work['stats']['language']: return 1
        if 'luck' in work['stats'].keys() and gs.luck<work['stats']['luck']: return 1
        if 'badluck' in work['stats'].keys() and gs.badluck<work['stats']['badluck']: return 1
        if 'fame' in work['stats'].keys() and gs.fame<work['stats']['fame']: return 1
        ma.player_energy+=work['stats']['energy']
        earn=work['stats']['earn']
        ma.earn+=earn
        print(f'energy...{work["stats"]["energy"]}\nearn...+{earn}')
    ...
    def event_value(self,Event):
        ...
        if Event=='PickMoney':
            ...
            gs.item.append('Clover')
            ev.bag('Clover')
            print('You picked clover!')
        elif Event=='Party':
            user=''
            while not user:
            ...
            gs.item.append('Rainbow')
            ev.bag('Rainbow')
            print('Over the rainbow!')
                
        elif Event=='DropMoney':
            if gs.luck+ma.luck<100:
                if gs.badluck+ma.badluck>0: loss=randint(3*(100-(gs.luck+ma.luck)),(3+gs.badluck+ma.badluck)*(100-(gs.luck+ma.luck)))
                else: loss=randint(3*(100-(gs.luck+ma.luck)),4*(100-(gs.luck+ma.luck)))
                gs.money-=loss
            ...
            if gs.luck+ma.luck<200:
                if gs.badluck+ma.badluck>0: loss=randint(5*(200-(gs.luck+ma.luck)),(5+gs.badluck+ma.badluck)*(200-(gs.luck+ma.luck)))
                else: loss=randint(5*(200-(gs.luck+ma.luck)),6*(200-(gs.luck+ma.luck)))
                gs.money-=loss
                print(f'Your house is on fire!    money...-{loss}')
            else:
                print(f'Your place is on fire,but someone helped you turn off the fire!    money...-0')
        else:
    ...
    def tips(self,stat):
        ...
        List+=[0 for x in range(gs.badluck+ma.badluck)]
        luck=sample(List,3)
        for i in luck: if i: result+=randint(1,gs.luck+ma.luck)*stat-(gs.badluck+ma.badluck)+gs.fame
        return result
...
while True:
    ...
    for _ in run_list:
        ...
        if days>=3 and not ma.rest and gs.money>0: ev.choose_event()

        tax=round(days*gs.tax_mul[0]+ma.earn*gs.tax_add[0],2)
        if days%5==0: 
            tax=round(days*gs.tax_mul[1]+ma.earn*gs.tax_add[1],2)
